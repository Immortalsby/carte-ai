...
- **LLM 安全护栏不可妥协**：6 条硬约束（不发明菜品/价格/过敏原/卡路里，过敏缺失强制提醒，仅输出 valid JSON）必须在所有 LLM 路径（Anthropic/OpenAI）和降级路径（本地规则）中一致执行。
...
| Anthropic Foundry | 1 | 出站 | LLM 主路（推荐增强 + OCR） |
| OpenAI API | 1 | 出站 | LLM 兜底 |
