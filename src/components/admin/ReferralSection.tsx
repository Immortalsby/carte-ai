...
export function ReferralSection({ locale }: { locale: AdminLocale }) {
  ...
  return (
    <div className="mt-8 rounded-xl border p-6">
      ...
      <p className="mt-1 text-sm text-gray-500">{t.referralDesc}</p>
      ...
      {data.permanentFree && (
        <div className="mt-4 flex items-center gap-2 rounded-lg bg-green-50 px-4 py-3 text-sm font-medium text-green-700">
          ...
          {t.referralPermanentFree}
        </div>
      )}

      {/* Stats */}
      <div className="mt-5">
        <h3 className="text-sm font-medium text-gray-700">{t.referralStats}</h3>
        <div className="mt-2 flex gap-6">
          <div className="flex items-center gap-2">
            <Users size={18} className="text-gray-400" />
            <span className="text-sm text-gray-600">
              {t.referralTotalInvited}: <strong>{data.totalReferred}</strong>
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Star size={18} className="text-yellow-500" />
            <span className="text-sm text-gray-600">
              {t.referralQualified}: <strong>{data.qualifiedReferred}</strong>
            </span>
          </div>
        </div>
      </div>
      ...
      {!data.permanentFree && (
        <div className="mt-4">
          <p className="text-xs text-gray-500">{t.referralProgress(data.qualifiedReferred)}</p>
          ...
          <p className="mt-2 text-xs text-gray-400">{t.referralRewardDesc}</p>
        </div>
      )}
    </div>
  );
