...
export function MenuEditor({ menu: initialMenu, slug, version, cuisine, locale = "en", isNewImport, onReImport }: MenuEditorProps) {
  ...
  function renameCategory(oldKey: string, newKey: string) {
    const normalized = newKey.trim().toLowerCase().replace(/\s+/g, "_");
    if (!normalized || normalized === oldKey || categoryOrder.includes(normalized)) return;
    // Update all dishes in this category
    setMenu((prev) => ({
      ...prev,
      dishes: prev.dishes.map((d) =>
        d.category === oldKey ? { ...d, category: normalized as MenuCategory } : d,
      ),
      categoryLabels: (() => {
        const labels = { ...(prev.categoryLabels ?? {}) };
        // Move label from old key to new key
        if (labels[oldKey]) {
          labels[normalized] = labels[oldKey];
          delete labels[oldKey];
        }
        // Set display label from user input
        const display = newKey.trim().charAt(0).toUpperCase() + newKey.trim().slice(1);
        labels[normalized] = { en: display, fr: display, zh: display };
        return labels;
      })(),
    }));
    // Update customCategories
    setCustomCategories((prev) =>
      prev.map((c) => (c === oldKey ? normalized : c)),
    );
  ...
  function deleteCategory(category: string) {
    // Remove all dishes in this category
    setMenu((prev) => ({
      ...prev,
      dishes: prev.dishes.filter((d) => d.category !== category),
    }));
                  ...
                  <div className="flex items-center gap-2">
                    {renamingCategory === category ? (
                      <input
                        autoFocus
                        type="text"
                        defaultValue={getCategoryLabel(category)}
                        placeholder={tAny.renameCategoryPlaceholder}
                        className="w-40 rounded-md border border-border bg-background px-2 py-1 text-sm font-bold uppercase tracking-wider text-foreground"
                        onKeyDown={(e) => {
                          if (e.key === "Enter") renameCategory(category, (e.target as HTMLInputElement).value);
                          if (e.key === "Escape") setRenamingCategory(null);
                        }}
                        onBlur={(e) => {
                          const val = e.target.value.trim();
                          if (val && val.toLowerCase().replace(/\s+/g, "_") !== category) {
                            renameCategory(category, val);
                          } else {
                            setRenamingCategory(null);
                          }
                        }}
                    ...
                    {!renamingCategory && !builtinCategoryOrder.includes(category as MenuCategory) && (
                      <>
                        <button
                          type="button"
                          onClick={() => setRenamingCategory(category)}
                          title={tAny.renameCategory}
                        ...
                        {confirmingCategoryDelete === category ? (
                          <span className="flex items-center gap-1 text-xs">
                            <span className="text-red-500">
                              {items.length > 0
                                ? (t.deleteCategoryConfirm as (n: number) => string)(items.length)
                                : tAny.deleteCategoryEmpty}
                            </span>
                            <button
                              type="button"
                          ...
                          <button
                            type="button"
                            onClick={() => setConfirmingCategoryDelete(category)}
                            title={tAny.deleteCategory}
